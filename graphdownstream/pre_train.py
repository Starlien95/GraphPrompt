...
from utils import anneal_fn, get_enc_len, pretrain_load_data, \
    get_linear_schedule_with_warmup,bp_compute_abmae,compareloss,compareloss_,split_and_batchify_graph_feats
'''from cnn import CNN
from rnn import RNN
from txl import TXL
...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        reg_loss = compareloss_(_pred,_sample,train_config["temperature"])
...
def evaluate(model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_loader):
            ...
            reg_loss = compareloss_(pred, train_config["temperature"])
            reg_loss_item = reg_loss.item()
