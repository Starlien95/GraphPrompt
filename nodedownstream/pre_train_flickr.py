...
def train(model, optimizer, scheduler, device,graph,  config, epoch, logger=None, writer=None,self_weight = 1):
    epoch_step = 1
...
if __name__ == "__main__":
    ...
    for weight in range(len(adj_self_weight)):
        for times in range(5):
            ...
            plt.savefig('epoch_loss%s_time%d.png'%(record[adj_self_weight[weight]],times))
            print("best:")
            print(train_config["epochs"] - 1)
            best_epoch = train_config["epochs"] - 1
            model.load_state_dict(torch.load(os.path.join(save_model_dir, 'epoch%d.pt' % (best_epoch))))
            torch.save(model.state_dict(), os.path.join(save_model_dir, "best_local_%s_times%d.pt"%(record[adj_self_weight[weight]],times)))
